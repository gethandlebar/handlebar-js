...
export function resultToSignalSchema(
	key: string,
	result: SignalResult,
): Signal | undefined {
	try {
...
export class SignalRegistry {
	...
	private bind(binding: SignalBinding, env: SignalEvalEnv): unknown {
		switch (binding.from) {
			...
			case "toolTag": {
				const tags = (env.call.tool.categories ?? []).map((t) =>
					t.toLowerCase(),
				);
			...
			case "subject": {
				const matches = env.subjects.filter(
					(s) =>
						s.subjectType === binding.subjectType &&
						(binding.role ? s.role === binding.role : true),
				);
