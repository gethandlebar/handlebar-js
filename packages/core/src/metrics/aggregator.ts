...
export class AgentMetricCollector {
	...
	addInbuilt(kind: InbuiltMetricKind, delta: number, unit?: string) {
		...
		this.inbuilt[kind] = {
			value: prev + delta,
			unit: unit ?? this.inbuilt[kind]?.unit,
		};
	}

	setCustom(kind: string, value: number, unit?: string) {
		const isInbuilt = InbuiltAgentMetricKindSchema.safeParse(kind);
		if (isInbuilt.success) {
			throw new Error(
				`Custom metric kind "${kind}" collides with inbuilt metric name`,
			);
		}

		if (!validateMetricKey(kind)) {
			throw new Error(`Invalid custom metric key "${kind}"`);
		}
	...
	addCustom(kind: string, delta: number, unit?: string) {
		...
		this.custom[kind] = {
			value: prev + delta,
			unit: unit ?? this.custom[kind]?.unit,
		};
	}
	...
	toEventPayload(opts: { aggregate?: boolean } = { aggregate: false }):
		...
		| undefined {
		const inbuiltEntries = Object.entries(this.inbuilt).filter(
			([, v]) => v && validateMetric(v),
		);
		const customEntries = Object.entries(this.custom).filter(
			([, v]) => v && validateMetric(v),
		);
