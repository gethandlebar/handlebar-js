...
export const RuleSelectorSchema = z
  ...
  .strict()
  .refine((v) => !!v.tool, "selector.tool is required (rule will never evaluate otherwise)");
...
export const RuleSpecSchema = RuleSchema.omit({ id: true, policyId: true }).describe("Definable Rule spec that can be inserted into Handlebar API");
