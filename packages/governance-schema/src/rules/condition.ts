...
import { RequireSubjectConditionSchema, SignalConditionSchema } from "./signals";
...
const BaseRuleConditionSchema = z.union([ToolNameConditionSchema,
  ToolTagConditionSchema,
...
const AndConditionSchema = z.object({ kind: z.literal("and"), all: z.array(BaseRuleConditionSchema).min(1) }).strict();
const OrConditionSchema = z.object({ kind: z.literal("or"), any: z.array(BaseRuleConditionSchema).min(1) }).strict();
const NotConditionSchema = z.object({ kind: z.literal("not"), not: BaseRuleConditionSchema }).strict();
...
export const RuleConditionSchema = z.union([BaseRuleConditionSchema, AndConditionSchema, OrConditionSchema, NotConditionSchema]);
