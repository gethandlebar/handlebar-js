...
export const ToolResultEventSchema = AuditEnvelopeSchema.extend({
  ...
  data: z.object({
    tool: z.object({ name: z.string(), categories: z.array(z.string()).optional()}),
    outcome: z.enum(["success", "error"]),
    durationMs: z.number().min(0).optional(), // tool runtime
    counters: CountersSchema.optional(),
    error: z.object({ name: z.string().optional(), message: z.string().optional(), stack: z.string().optional() }).optional(),
